...
def test_accept_makes_final(make_api_client):
    ...
    body = client.post(f"/review/{fid}/accept").json()
    assert body["status"] == "accepted"


def test_accept_requires_draft(make_api_client):
    client, _ = make_api_client(FakeLLMProvider([]))
    flags = client.get("/variances", params={"current_period": 6}).json()  # seeds detected items
    fid = flags[0]["flag_id"]
    assert client.post(f"/review/{fid}/accept").status_code == 409  # still detected


def test_edit_retains_original_and_records(make_api_client):
    client, fid = _to_drafted(make_api_client)
    body = client.post(f"/review/{fid}/edit", json={"edited_text": "Controller wording."}).json()
...
def test_edit_on_accepted_reverts_to_drafted(make_api_client):
    client, fid = _to_drafted(make_api_client)
    client.post(f"/review/{fid}/accept")  # → accepted
    body = client.post(f"/review/{fid}/edit", json={"edited_text": "Tweak."}).json()
...
def test_dismiss(make_api_client):
    client, fid = _to_drafted(make_api_client)
    body = client.post(f"/review/{fid}/dismiss").json()
    assert body["status"] == "dismissed"


def test_every_action_audited(make_api_client):
    client, fid = _to_drafted(make_api_client)
    client.post(f"/review/{fid}/accept")
    history = client.get(f"/review/{fid}/history").json()
